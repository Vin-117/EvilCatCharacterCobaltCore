...
public class AExhaustSelect : CardAction
{
    ...
    public static Spr AExhaustSelectSpr; //Define icon var
    ...
    public override List<Tooltip> GetTooltips(State s)
    {
        ...
        if (count == 1)
        {
            return
            [
                new GlossaryTooltip(key: "AExhaustSelect")
                {
                    Icon = AExhaustSelectSpr,
                    Title = ModEntry.Instance.Localizations.Localize(["action", "AExhaustSelect", "title"]),
                    TitleColor = Colors.card,
                    Description = ModEntry.Instance.Localizations.Localize(["action", "AExhaustSelect", "desc_single"])
                },
            ];
        }
        else 
        {
            ...
            [
                new GlossaryTooltip(key: "AExhaustSelect")
                {
                    ...
                    Title = ModEntry.Instance.Localizations.Localize(["action", "AExhaustSelect", "title"]),
                    TitleColor = Colors.card,
                    Description = ModEntry.Instance.Localizations.Localize(["action", "AExhaustSelect", "desc"], new { cnt = count })
                },
            ];
        }
    }
